...
public class Google_API extends Abstract_Class {
    ...
    String apiKey = "AIzaSyDfdKybLDVSBhVEQV631cvrUXWF0pbMkTA";
    ...
    public void googleSearchApiTest() throws InterruptedException {
        ...
        Response Resp=
                given()
                        ...
                        .queryParam("cx","3f3e1f86c07aaf690")
        ...
        if (linkCount.size() >= 1){
            System.out.println("link count is greater than or equal to 1. The link count found is " + linkCount.size());
        } else {
            System.out.println("Link count is not valid " + linkCount.size());
        }


        //run a loop and navigate to top 5 link and verify the title matches
        for (int i =0; i < 5; i++){
